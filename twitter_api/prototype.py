...
def search_for_hashtags(consumer_key, consumer_secret, access_token, access_token_secret, hashtag_phrase):
    ...
    with open(hashtag_phrase + '.csv', 'w', encoding='utf-8') as file:
        ...
        for tweet in tweepy.Cursor(api.search_tweets, q=hashtag_phrase+' -filter:retweets',
            ...
            w.writerow([tweet.created_at,  # date of tweet
                        tweet.user.location,  # the user's location if they provide it
                        tweet.full_text.replace('\n', ' ').encode(
                            'utf-8'),  # full text of tweet
                        # the user's nickname ("handle")
                        tweet.user.screen_name.encode('utf-8'),
                        # the list of contained hashtags
                        [e['text']
                            for e in tweet._json['entities']['hashtags']],
                        tweet.user.followers_count])  # the user's amount of followers
